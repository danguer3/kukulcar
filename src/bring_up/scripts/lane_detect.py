...
def canny(image):
    ...
    blur = cv2.GaussianBlur(gray, (7, 7), 0)                                    # APPLY BLUR FILTER TO REDUCE NOISE
    canny = cv2.Canny(blur, 60, 150)                                            # DETECT EDGES WITH CANNY
...
def region_of_interest(image):
	...
	(150,240),
	(490,240),
	(640,360),                                                               # FIRST POINT, LEFT-BOTTOM
        (640, 480)                                                             # SECOND POINT, RIGHT-BO                                                              # THIRD POINT, CENTER
    ]])
    mask = np.zeros_like(image)
    cv2.fillPoly(mask, triangle, 255)
    masked_img = cv2.bitwise_and(image, mask)
    #cv2.imshow("msk",masked_img)
    return masked_img
...
def main():
	...
	#print("left",polar_left_border)
	#print("right",polar_right_border)
